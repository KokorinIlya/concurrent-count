...
abstract class AbstractArrayQueue<T : TimestampedValue>(bufferSize: Int, initValue: T) : AbstractQueue<T> {
    ...
    protected companion object {
        ...
        val headUpdater = AtomicIntegerFieldUpdater.newUpdater(
            AbstractArrayQueue::class.java,
            "head"
        )

        @Suppress("HasPlatformType")
        val tailUpdater = AtomicIntegerFieldUpdater.newUpdater(
            AbstractArrayQueue::class.java,
            "tail"
        )
    }
    ...
    override fun popIf(timestamp: Long): Boolean {
        while (true) {
            ...
            if (curHead == curTail && get(curHead)!!.timestamp != timestamp) {
                return false
            } else if (headUpdater.compareAndSet(this, curHead, curHead + 1)) {
                return true
            }
        }
    }
